...
function ObjRHSBatchQuadraticModel(
  ...
  data = QPData(
    ...
    c0 = qp.data.c0,
    _v = qp.data._v,
    regularize = qp.data.regularize,
    selected = qp.data.selected,
    σ = qp.data.σ,
  )
...
function NLPModels.obj!(bqp::ObjRHSBatchQuadraticModel{T}, bx::AbstractMatrix, bf::AbstractVector) where {T}
  ...
  _mul_hessian!(bqp._HX, bqp.data.Q, bx)
  bqp._HX .*= T(0.5)
  bqp._HX .+= bqp.c_batch
...
function grad_subset!(
  ...
  _mul_hessian!(view(bqp._HX, :, 1:length(roots)), bqp.data.Q, bx)
...
function NLPModels.hess_structure!(
  ...
  return _copy_sparse_structure!(hrows, hcols, bqp.data.Q)
end
